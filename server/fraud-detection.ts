...
export async function detectFraud(
  ...
  // Rule 2: Very large transaction (>$50,000)
  if (amount > 50000) {
    risks.push({
  ...
  if (last24Hours.length >= 10) {
    risks.push({
      reason: "High transaction velocity (10+ transactions in 24 hours)",
      score: 70,
    });
  }
  ...
  if (last5Minutes.length >= 5) {
    risks.push({
      reason: "Multiple rapid transactions (5+ in 5 minutes)",
      score: 80,
    });
  }
  ...
  if (merchantFrequency.length === 0 && amount > 1000) {
    risks.push({
  ...
  const isFraudulent = maxRiskScore >= 70;
